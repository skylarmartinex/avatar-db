...
import { Package, ArrowLeft, ExternalLink, FileJson, Palette } from 'lucide-react';
...
export default function StylePackDetailPage() {
    ...
    useEffect(() => {
        async function loadData() {
            try {
                // Load pack data
                const packRes = await fetch(`/api/module?dim=STY&code=${packCode}`);
                ...
                if (promptsData.prompts && pack) {
                    const matching = promptsData.prompts.filter((p: PromptFile) => {
                        return (
                            p.canonicalId.includes(`SC-${pack.SC}`) &&
                            p.canonicalId.includes(`ST-${pack.ST}`)
                        );
                    });
            ...
            <div className="space-y-8">
                ...
                <div className="space-y-6">
                    <div className="flex items-start justify-between gap-8">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <Card className="bg-zinc-900 border-white/5">
                                        <CardContent className="p-6 space-y-2">
                                            <p className="text-xs font-mono text-zinc-600 uppercase">Scene</p>
                                            <p className="text-lg font-bold text-white">{getLabel('SC', packData.SC!)}</p>
                                            <p className="text-xs font-mono text-zinc-500">{packData.SC}</p>
                                        </CardContent>
                                    </Card>
                                    <Card className="bg-zinc-900 border-white/5">
                                        <CardContent className="p-6 space-y-2">
                                            <p className="text-xs font-mono text-zinc-600 uppercase">Outfit</p>
                                            <p className="text-lg font-bold text-white">{getLabel('ST', packData.ST!)}</p>
                                            <p className="text-xs font-mono text-zinc-500">{packData.ST}</p>
                                        </CardContent>
                                    </Card>
                                </div>
                            </div>
                        ...
                        <Link href={`/builder?SC=${packData.SC}&ST=${packData.ST}`}>
